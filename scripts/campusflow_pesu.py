...
def parse_pesu_results_html(html: str):
    ...
    if not raw_html.strip() or BeautifulSoup is None:
        return result
...
def normalize_portal_select_value(value: Any):
    text = clean_text(value)
...
def parse_portal_semester_options(raw: Any):
    options = []
    text = str(raw or "").strip()
    ...
    try:
        ...
        for option in soup.find_all("option"):
            value = normalize_portal_select_value(option.get("value"))
    ...
    def walk(value: Any):
        ...
        option_value = (
            value.get("value")
            or value.get("id")
            or value.get("batchClassId")
            or value.get("classId")
            or value.get("semId")
            or value.get("semesterId")
        )
        ...
        if option_value is not None:
            options.append(
                {
                    "value": normalize_portal_select_value(option_value),
                    "label": clean_text(option_label or option_value),
                }
            )
...
def pick_portal_attendance_semester(options: list[dict[str, str]], semester_number: int | None):
    if not options:
        return None

    if semester_number is not None:
        wanted = str(semester_number).strip().lower()

        for option in options:
            value = clean_text(option.get("value")).lower()
            label = clean_text(option.get("label")).lower()

            if (
                value == wanted
                or label == wanted
                or f"sem-{wanted}" in label
                or f"sem {wanted}" in label
                or f"semester-{wanted}" in label
                or f"semester {wanted}" in label
            ):
                return option
...
def parse_number_from_attendance_text(value: Any):
    text = clean_text(value)
...
def parse_attendance_ratio(value: Any):
    text = clean_text(value)
    ...
    return {
        "attended": int(float(match.group(1))),
        "total": int(float(match.group(2))),
    }


def portal_attendance_percentage(attended: int, total: int):
    if total <= 0:
        return 0
...
def find_attendance_header_index(headers: list[str], patterns: list[str]):
...
def parse_portal_attendance_table(html: str):
    subjects = []
    ...
    for table in soup.find_all("table"):
        ...
        code_index = find_attendance_header_index(
            lowered_headers,
            ["course code", "subject code", "code"],
        )
        name_index = find_attendance_header_index(
            lowered_headers,
            ["course name", "subject name", "subject", "course", "name", "title"],
        )
        attended_index = find_attendance_header_index(
            lowered_headers,
            ["attended", "present", "classes attended", "class attended"],
        )
        total_index = find_attendance_header_index(
            lowered_headers,
            ["total", "conducted", "held", "classes conducted", "class conducted"],
        )
        for row in rows:
            cells = [
                clean_text(cell.get_text(" ", strip=True))
            ...
            code = cells[code_index] if 0 <= code_index < len(cells) else ""
            name = cells[name_index] if 0 <= name_index < len(cells) else ""

            if not code:
                code_match = re.search(r"\b[A-Z]{2,6}\d{2}[A-Z]{2}\d{3}[A-Z]?\b", row_text)
                if code_match:
            ...
            if not name:
                possible_names = [
                    ...
                    and not parse_attendance_ratio(cell)
                ]
            ...
            for cell in cells:
                ratio = parse_attendance_ratio(cell)
            ...
            if ratio:
                ...
                attended = (
                    parse_number_from_attendance_text(cells[attended_index])
                    if 0 <= attended_index < len(cells)
                    else 0
                )
                total = (
                    parse_number_from_attendance_text(cells[total_index])
                    if 0 <= total_index < len(cells)
                    else 0
                )

            percentage = portal_attendance_percentage(attended, total)
            ...
            clean_code = clean_code or f"SUBJECT-{len(subjects) + 1}"
    ...
    for subject in subjects:
        if subject["code"] not in seen:
            seen[subject["code"]] = subject

    return list(seen.values())


async def fetch_portal_attendance_semester_options(pesu: Any):
    http_client = find_internal_http_client(pesu)

    if http_client is None:
        raise RuntimeError("Could not find authenticated PESU HTTP client.")

    response = await maybe_await(
        http_client.get(
            "https://www.pesuacademy.com/Academy/a/studentProfilePESU/getStudentSemestersPESU",
            headers=make_xhr_headers(pesu),
        )
    )

    raw = await response_to_text(response)

    return parse_portal_semester_options(raw)


async def fetch_portal_attendance_for_semester(pesu: Any, batch_class_id: str):
    http_client = find_internal_http_client(pesu)

    if http_client is None:
        raise RuntimeError("Could not find authenticated PESU HTTP client.")

    normalized_batch_class_id = normalize_portal_select_value(batch_class_id)

    form_data = {
        "controllerMode": "6407",
        "actionType": "8",
        "batchClassId": normalized_batch_class_id,
        "menuId": "660",
    }
    ...
    if os.environ.get("CAMPUSFLOW_DUMP_ATTENDANCE_HTML") == "1":
        dump_path = "/tmp/campusflow-portal-attendance-table.html"
        with open(dump_path, "w", encoding="utf-8") as dump_file:
            dump_file.write(str(html or ""))

        print(
            json.dumps(
                {
                    "debug": "portal-attendance-table-html-dumped",
                    "path": dump_path,
    ...
    return parse_portal_attendance_table(html)


async def fetch_portal_attendance(pesu: Any, semester_number: int | None = None):
    options = await fetch_portal_attendance_semester_options(pesu)
    selected = pick_portal_attendance_semester(options, semester_number)

    if not selected:
        return []

    subjects = await fetch_portal_attendance_for_semester(pesu, selected["value"])

    if os.environ.get("CAMPUSFLOW_DEBUG_ATTENDANCE_RAW") == "1":
        print(
            json.dumps(
                {
                    "debug": "portal-attendance-final",
                    "selectedSemester": selected,
                    "semesterOptions": options,
                    "subjectCount": len(subjects),
                    "subjectsPreview": subjects[:5],
                }
            ),
            file=sys.stderr,
        )
...
def find_first_attendance_rows(value: Any):
    if isinstance(value, list):
        return value
    ...
    for nested in value.values():
        rows = find_first_attendance_rows(nested)
...
def pick_attendance_rows(raw_attendance: Any, semester_number: int | None = None):
    ...
    if selected_semester and isinstance(raw_attendance.get(selected_semester), list):
        return raw_attendance.get(selected_semester, [])

    latest_semester = choose_latest_semester_key(raw_attendance)

    if latest_semester and isinstance(raw_attendance.get(latest_semester), list):
        return raw_attendance.get(latest_semester, [])

    return find_first_attendance_rows(raw_attendance)


def first_attendance_value(data: dict[str, Any], keys: list[str]):
    for key in keys:
        value = data.get(key)
...
def normalize_attendance_subject(subject: Any, index: int):
    if not isinstance(subject, dict):
        return None

    attendance = subject.get("attendance") or subject.get("attendanceDetails") or {}

    if not isinstance(attendance, dict):
        attendance = {}

    code = clean_text(
        first_attendance_value(
            subject,
            ["code", "courseCode", "course_code", "subjectCode", "subject_code"],
        )
        or ""
    )
    name = clean_text(
    ...
    attended = normalize_attendance_count(
        first_attendance_value(
            attendance,
            [
                "attended",
    ...
    total = normalize_attendance_count(
        first_attendance_value(
            attendance,
            [
                "total",
    ...
    code = code or clean_text(subject.get("id")) or f"SUBJECT-{index + 1}"
    name = name or code
    fallback_id = code or f"attendance-{index}"
    ...
    return {
        ...
        "id": subject.get("id") or fallback_id,
    }


def normalize_attendance(raw_attendance: Any, semester_number: int | None = None):
    subjects = pick_attendance_rows(raw_attendance, semester_number)
    normalized = []

    for index, subject in enumerate(subjects):
        normalized_subject = normalize_attendance_subject(subject, index)

        if normalized_subject is not None:
            normalized.append(normalized_subject)
...
async def main():
    ...
    try:
        ...
        attendance_error = (
            ...
            else attendance_result.get("error") or "Could not fetch wrapper attendance."
        )
        timetable = {
            "days": ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"],
            "roomId": None,
        ...
        if not attendance:
            portal_attendance_result = await safe_call(
                lambda: fetch_portal_attendance(pesu, semester_number)
            )

            if portal_attendance_result["ok"] and portal_attendance_result["data"]:
                attendance = portal_attendance_result["data"]
                attendance_error = None
            elif portal_attendance_result["ok"]:
                attendance_error = (
                    attendance_error or "PESU attendance returned no subjects."
                )
            else:
                portal_error = (
                    portal_attendance_result.get("error")
                    or "Portal attendance fallback failed."
                )
                attendance_error = (
                    f"{attendance_error}; {portal_error}"
                    if attendance_error
                    else portal_error
                )
        ...
        debug_log(
            {
                "debugAttendanceFinalSubjectCount": len(attendance),
                "debugAttendanceWrapperOk": attendance_result["ok"],
                "debugAttendanceError": attendance_error,
            }
        )
